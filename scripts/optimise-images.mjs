...
for (const f of walk(ROOT)) {
  ...
  if (!usesAlpha) {
    if (f.includes('logo-anchor')) {
      ...
      await sharp(f).trim({ threshold: 8 }).png({ compressionLevel: 9, palette: false }).toFile(out);
      const m2 = await sharp(out).metadata();
      before[f] = bytes; after[f] = statSync(out).size;
      unlinkSync(f); renameSync(out, f);
      log.push(`  logo  ${meta.width}x${meta.height} → ${m2.width}x${m2.height}   ${Math.round(bytes/1024)} → ${Math.round(statSync(f).size/1024)} KB`);
    } else {
      // Opaque photograph stored as PNG → JPEG
      const out = f.replace(/\.png$/i, '.jpg');
      await sharp(f).flatten({ background: '#ffffff' })
        .jpeg({ quality: 88, mozjpeg: true, chromaSubsampling: '4:4:4' }).toFile(out);
      before[f] = bytes; after[out] = statSync(out).size;
      unlinkSync(f);
      log.push(`  jpeg  ${f.replace(ROOT+'/','')} → .jpg   ${Math.round(bytes/1024)} → ${Math.round(statSync(out).size/1024)} KB`);
    }
  } else {
    ...
    if (nb < bytes) {
      before[f] = bytes; after[f] = nb; unlinkSync(f); renameSync(out, f);
      log.push(`  png   ${f.replace(ROOT+'/','')}   ${Math.round(bytes/1024)} → ${Math.round(nb/1024)} KB`);
    } else { unlinkSync(out); }
  }
}
